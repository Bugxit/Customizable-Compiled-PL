...
def getKeyword(line : str, lineNumber : int | None = 'Not specified') -> str:
    ...
    keywordsList = ['write', 'int', 'var', 'let', 'transform', 'pass', 'function', 'call', 'class', 'for', 'if', 'elif', 'else', 'break']
...
def executeFile(file : dict, function : str):
    hasToBreak = ""
    lastCondition = None
    forbiddenLines = []
    for position, lineNumber in enumerate(file.keys()):
        if lineNumber not in forbiddenLines:
            ...
            if keyword not in ['for', 'if', 'elif', 'else', 'break']:
                lastCondition = None
                ...
                for forVar in listObject(splitedLine[3], function, lineNumber):
                    ...
                    hasToBreak = executeFile(forFile, function)
                    if hasToBreak == "breaker":
                        break
                ...
                if boolObject(splitedLine[1][:-1], function, lineNumber) and keyword == 'if':
                    lastCondition = True
                    hasToBreak = executeFile(ifFile, function)
...
def reassembleWords(textList : list) -> list:
    for position, letter in enumerate(textList):
        ...
        elif letter == '=' and textList[position+1] == '=':
            textList[position] = '=='
    return textList
...
def floatObject(text : str, function : str, lineNumber) -> float:
    ...
    if '%' in text:
        splitedText = text.split("%", 1)
        splitedText = getSplitedOperation(splitedText)
        return floatObject(splitedText[0][0]+str(int(floatObject(splitedText[0][1], function, lineNumber))%int(floatObject(splitedText[1][0], function, lineNumber)))+splitedText[1][1], function, lineNumber)
...
def listObject(text : str, function : str, lineNumber : int) -> list:
    if "range" in text:
        return [x for x in range(intObject(text[6:(len(text)-2)], function, lineNumber))]
...
def keyword_var(line : str, lineNumber : int, function : str) -> None:
    ...
    if splitedLine[2] != "=":
        ...
        error("You cannot modify a variable wich does not exist", lineNumber)
    try:
        if varsInfo[function][splitedLine[1].lower()]["type"] == "int":
            varsInfo[function][splitedLine[1].lower()]["value"] = int(intObject(splitedLine[3][:-1], function, lineNumber))
    except:
        error("The value specified is not of the correct type", lineNumber)
